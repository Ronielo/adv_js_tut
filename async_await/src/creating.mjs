...
export function allPromises() {
    ...
    let addressType = axios.get("http://localhost:3000/adressTypes")

    Promise.all([categories, statuses, userTypes, addressType]).then(([cat, stat, type, address]) => {
        setText("");
...
export function race() {
    ...
    Promise.race([users, backup]).then(users => {
